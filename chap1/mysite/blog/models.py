...
""" 
? Python object (class) is a collection of data and methods.
...
? ach attribute of the class represents a database field.

? When you create a model, Django will provide you with a practical API to query objects in the database easily.
? We will define the database models for our blog application. Then, we will generate the database mi-
? grations for the models to create the corresponding database tables. When applying the migrations,
? Django will create a table for each model defined in the models.py file of the application. """


class PublishedManager(models.Manager):
    """
    ? creating a custom manager for the published model
    """
...
class Post(models.Model):
    ...
    slug = models.SlugField(max_length=250) # translates to VARCHAR in db, used to build a beautiful URL
    ...
    ? We have imported the project’s settings and we have added an author field to the Post model. 
